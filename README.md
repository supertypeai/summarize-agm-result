...
- Supports two summary modes:
  ...
  - `--pubex`: Public Expose summaries with `Company Update` and `QnA` sections
...
- Produces JSON output with this structure:
  ...
  - `tags`: taxonomy-based labels from `agms_tags_classification.txt`, e.g. `["dividend", "board and management", "corporate governance"]`
   - `summary`:
      - AGMS mode: agenda lines only (`Agenda #1: ...` to `Agenda #n: ...`)
      - PubEx mode:
        - `Company Update:` key factual updates (strategy/performance/outlook/operations/financing/risk)
       - `QnA:` Q&A lines (`Q&A #1: Q: ... A: ...`)
...
OPENROUTER_VISION_MODEL=qwen/qwen3.6-plus
OPENROUTER_TIMEOUT_SEC=240
OPENROUTER_VISION_MAX_PAGES=40
OPENROUTER_MAX_IMAGE_BYTES=28000000
OPENROUTER_MAX_PAGES_PER_REQUEST=4
OPENROUTER_MAX_OUTPUT_TOKENS=12000
OPENROUTER_HTTP_REFERER=
OPENROUTER_X_TITLE=meeting-summarizer
IDX_PUBEX_QNA_KEYWORD=laporan hasil public expose
IDX_PUBEX_COMPANY_UPDATE_KEYWORD=materi public expose
...
- For PubEx Company Update in IDX mode, set OpenRouter multimodal envs (`OPENROUTER_API_KEY`, `OPENROUTER_VISION_MODEL`).
- `OPENROUTER_MAX_IMAGE_BYTES` caps total rendered image bytes per OpenRouter request (default `28000000`) and auto-batches pages to avoid 413 payload errors.
- `OPENROUTER_MAX_PAGES_PER_REQUEST` limits pages per OpenRouter request (default `4`) to reduce response truncation risk.
- `OPENROUTER_MAX_OUTPUT_TOKENS` sets OpenRouter response budget per request (default `12000`).
...
- `--pubex`: summary with `Company Update` + `QnA` for Public Expose documents
...
```bash
meeting-summarizer --from-idx --pubex --idx-keyword "laporan hasil public expose" --api gemini
```

In `--from-idx --pubex` mode:
- QnA source defaults to keyword `laporan hasil public expose` (or `--idx-keyword`).
- Company Update source defaults to keyword `materi public expose` (`IDX_PUBEX_COMPANY_UPDATE_KEYWORD`).
- Pairing rule: match same company code, with Company Update date on/before QnA date and max gap `14` days.
- QnA is summarized from extracted transcript.
- Company Update extraction is generated by OpenRouter multimodal image input and normalized into `PAGE <n>: ...` lines.
- The selected `--api` model then summarizes the page-wise extraction into the final `Company Update:` section.
- QnA PDF attachment selection uses QnA keyword hits only (no size/page weighting).
- Company Update PDF attachment selection always favors the largest/page-rich attachment.
- Final PubEx output is merged into `Company Update:` and `QnA:` sections.

Useful flags for IDX mode:

- `--idx-keyword`: override `IDX_KEYWORD` (supports wildcard `*` and `?`, e.g. `ringkasan risalah*MAPB*`)
- `--idx-company`: filter by company code (e.g. `BBCA` or `BBCA.JK`, supports wildcard `*` and `?`, can be repeated or comma-separated)
...
- `--idx-debug-dump`: write fetched IDX announcements + filter-stage results to a JSON file
...
- `IDX_PUBEX_QNA_KEYWORD`: default PubEx QnA keyword (`laporan hasil public expose`)
- `IDX_PUBEX_COMPANY_UPDATE_KEYWORD`: default PubEx Company Update keyword (`materi public expose`)
- `OPENROUTER_API_KEY`: required for PubEx Company Update multimodal extraction in IDX mode
- `OPENROUTER_VISION_MODEL`: model used by OpenRouter for Company Update page-wise extraction
